...
async function authInit(){
    ...
    onAuthStateChanged(auth,u=>{user=u;if(u){setStatus("Firebase connected ✓",true);finish();}else setStatus("Firebase syncing…",false);updateAdmin();},e=>{console.warn("Auth state error:",e);setStatus("Firebase unavailable • PIN ready",false);finish();});
...
function updateAdmin(){
    ...
    if($("connectionStatus"))
        $("connectionStatus").textContent=user?"Firebase connected ✓":"Firebase syncing…";
}
...
function deviceInfo(){
    ...
    else if(/Samsung/i.test(ua)){brand='Samsung';model=model==='Browser'?'Samsung device':model;}
    else if(/Pixel/i.test(ua)){brand='Google';model=model==='Browser'?'Pixel':model;}
    else if(/OnePlus/i.test(ua)){brand='OnePlus';model=model==='Browser'?'OnePlus device':model;}
    else if(/Android/i.test(ua)){
        brand='Android';
        const m=ua.match(/Android[^;)]*;\s*(?:[a-z]{2}(?:-[A-Z]{2})?;\s*)?([^;)]+?)(?:\s+Build\/[^;)]+)?[;)]/i);
        if(m?.[1])model=m[1].trim();
    }
    const browser=/Edg\//i.test(ua)?'Edge':/CriOS|Chrome\//i.test(ua)?'Chrome':/FxiOS|Firefox\//i.test(ua)?'Firefox':/Safari/i.test(ua)&&!/Chrome|CriOS/i.test(ua)?'Safari':'Browser';
    return {brand,model,browser,platform:navigator.platform||'',language:navigator.language||'',screen:`${window.screen?.width||0}×${window.screen?.height||0}`};
}
...
async function audit(action,details={}){
    try{
        ...
        await addDoc(collection(db,AUDIT_COL),{
            ...
            deviceBrand:d.brand,deviceModel:d.model,browser:d.browser,platform:d.platform,language:d.language,screen:d.screen,clientTime:new Date().toISOString(),createdAt:serverTimestamp()
        });
...
function show(id){
    const pages=['modulePage','customerPageSection','searchSection','addSection','repairAddSection','repairSearchSection','secondAddSection','secondListSection','accessoryAddSection','accessoryListSection','homeDateWorkSection'];
    ['customerDetailModal','themeModal','scannerModal',...pages].forEach(x=>$(x)?.classList.add('hidden'));
    $(id)?.classList.remove('hidden');
    if(pages.includes(id)){$('homeModules')?.classList.add('hidden');$('homeCustomerCardWrap')?.classList.add('hidden');}
    else{$('homeModules')?.classList.remove('hidden');$('homeCustomerCardWrap')?.classList.remove('hidden');}
    setTimeout(()=>$(id)?.scrollIntoView({behavior:'smooth',block:'start'}),20);
}
function closeSpecific(id,e){e?.preventDefault();e?.stopPropagation();$(id)?.classList.add('hidden');const pages=['modulePage','customerPageSection','searchSection','addSection','repairAddSection','repairSearchSection','secondAddSection','secondListSection','accessoryAddSection','accessoryListSection','homeDateWorkSection'];if(!pages.some(x=>$(x)&&!$(x).classList.contains('hidden'))){$('homeModules')?.classList.remove('hidden');$('homeCustomerCardWrap')?.classList.remove('hidden');}}
...
async function saveRepair(e){e.preventDefault();if(enforceWriteLock('repairMessage'))return;const ids=['repairCustomerName','repairPhone','repairDevice','repairProblem','repairBy','repairTotal','repairPartsPrice'];for(const id of ids)if(!val(id)){ $(id)?.focus();return msg('repairMessage','सभी fields भरना जरूरी है.')}const phone=val('repairPhone').replace(/\D/g,'');if(!/^\d{10}$/.test(phone))return msg('repairMessage','10 digit customer phone number डालें.');const total=Number(val('repairTotal')||0),parts=Number(val('repairPartsPrice')||0),profit=total-parts;const saveBtn=$('repairForm')?.querySelector('button[type="submit"]');if(saveBtn)saveBtn.disabled=true;try{const ref=await addDoc(collection(db,REPAIR_COL),{customerName:val('repairCustomerName'),phone,device:val('repairDevice'),problem:val('repairProblem'),repairBy:val('repairBy'),total,partsPrice:parts,profit,payment:total,createdAt:serverTimestamp(),createdBy:user?.uid||null});await audit('repairing_add',{section:'Kabir Repairing Data',customerId:ref.id,customerName:val('repairCustomerName'),description:`Repairing added: ${val('repairProblem')}`,extra:{phone,device:val('repairDevice'),total,partsPrice:parts,profit}});$('repairForm').reset();$('repairProfit').textContent='₹0';msg('repairMessage','',true);showSuccessToast('Successfully Saved','Repairing data saved successfully')}catch(err){console.error(err);msg('repairMessage',err?.message||'Repairing save नहीं हुआ.')}finally{if(saveBtn)saveBtn.disabled=false}}
...
async function safeFirebaseStatus(text, ok=false){
...
// Main website startup is deliberately independent of Firebase.
// Firebase is a background sync service; it must never control the PIN/lock screen.
function startFirebaseBackground(){
    Promise.resolve().then(()=>firebaseReadyPromise).then(ready=>{
        if(!ready || !db){ safeFirebaseStatus("Offline mode • PIN ready",false); return; }
        return authInit();
    }).catch(err=>{
        console.warn("Firebase background startup skipped:",err);
        safeFirebaseStatus("Offline mode • PIN ready",false);
    });
}

function init(){
    setupPin();
    startFirebaseBackground();
    // PIN UI must never wait for Firebase. It uses cached/shared PIN when available.
    loadSharedPin().catch(e=>console.warn("Shared PIN load:",e));
